...
Creates a file containing framewise model predictions using the temporal machine-learning model
(takes 90 frames into account when making predictions) for the following script:
...
if __name__ == '__main__':
    ...
    for openface_filename in tqdm(csv_files_openface):
        ...
        for i in range(openface_feats.shape[0] - features_num_frames):
            ...
            all_features_video_temp.append(features)
        
        all_features_video_temp = np.array(all_features_video_temp)
        all_features_video_temp = normalize_3d_features(all_features_video_temp)
        ...
        params_test = {'batch_size': all_features_video_temp.shape[0],
                  'shuffle': False,
                  'drop_last': False}
        
        # Initialize the test set
        test_set = temporal_dataset(all_features_video_temp)
        ...
        with no_grad():
            ...
            for test_data in test_data_loader:
                ...
                prediction_confidences_temp = softmax(y_hat, axis=1)
                
        prediction_confidences_temp = np.array(prediction_confidences_temp, dtype=np.float32)
        ...
        write_name = os.path.join(output_dir, openface_filename.split('.')[0] + '_model_predictions.npy')
        
        # A list where all prediction confidences of looking at the face are added (temporal model)
        face_pred_confidences_temp = []
        ...
        for video_frame_index in range(num_frames_video - 1):
            ...
            if video_frame_index < (features_num_frames - 1):
                face_pred_confidences_temp.append(0)
            else:
                confidence_face = prediction_confidences_temp[video_frame_index - features_num_frames + 1][1]
                face_pred_confidences_temp.append(confidence_face)
                
        face_pred_confidences_temp = np.array(face_pred_confidences_temp, dtype=np.float32)
        np.save(write_name, face_pred_confidences_temp)
