...
Using the backward feature selection method, we rank each feature's importance and
...
class dataset_backward_selection(Dataset):

    def __init__(self, all_feature_indices, tested_feature, dropped_feature_indices, train_val_test='train',
                 feature_path='./feats', train_val_ratio=0.75, random_state=22):
        super().__init__()
        ...
        for feat_index in all_feature_indices:
            ...
            elif feat_index in dropped_feature_indices:
                features_to_be_randomized.append(feat_index)
...
if __name__ == '__main__':
    ...
    name_of_textfile = 'backward_selection_log.txt'
    ...
    best_score_of_previous_iteration = 0.61869 # The score of the model with all features in use
    ...
    while continue_feat_selection_process:
        max_score = -1
        worst_feat_index = None
        with open(name_of_textfile, 'a') as f:
            f.write(f'Dropped feature indices: {dropped_feature_indices} \n\n')
        ...
        for tested_feature in feature_indices_to_be_tested:
            ...
            test_set = dataset_backward_selection(all_feature_indices, tested_feature, dropped_feature_indices,
                                                  train_val_test='test')
            ...
            if testing_score > max_score:
                ...
                worst_feat_index = tested_feature
        ...
        with open(name_of_textfile, 'a') as f:
            f.write(f'Worst tested feature index: {worst_feat_index} \n')
            f.write(f'Score of worst tested feature: {max_score:7.5f} \n\n\n')
        
        # Add the feature to the dictionary of feature importances
        worst_score_difference_current_iteration = max_score - best_score_of_previous_iteration
        best_score_of_previous_iteration = max_score
        dict_of_feature_importances[worst_feat_index] = [max_score, worst_score_difference_current_iteration]
        
        # Find the index of the worst feature and delete that index for upcoming iterations
        dropped_feature_indices.append(worst_feat_index)
        worst_feat_position = np.where(feature_indices_to_be_tested == worst_feat_index)[0][0]
        feature_indices_to_be_tested = np.delete(feature_indices_to_be_tested, worst_feat_position)
        
        if len(dropped_feature_indices) == input_dim:
            continue_feat_selection_process = 0
    
    i = 1
    with open(name_of_textfile, 'a') as f:
        f.write('The results of the backward feature selection process: \n')
